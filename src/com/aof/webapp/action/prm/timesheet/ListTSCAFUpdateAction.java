 ...
 */
public class ListTSCAFUpdateAction extends BaseAction {
	public ActionForward perform(
		...
		Logger log = Logger.getLogger(ListTSCAFUpdateAction.class.getName());
		...
		try {
			...
			SQLExecutor sqlExec = new SQLExecutor(
						...
						String QryStr =   " select ptm.ts_period, " 
										+ " sum(ptd.ts_hrs_user), "
										+ " sum(case when ( pe.Billable = 'YES' and pm.proj_caf_flag = 'Y' and ptd.ts_cafstatus_confirm = 'N') then ptd.ts_hrs_user else 0 end)" 
						...
						if(result.size() != 0){        
							for(int i1 = 0, i2 = 0; i1 < MstrResult.size(); i1++){
								...
								if((al).get(0).equals(result.get(i2))){
									...
									i2 += 2;
									MstrResult.remove(i1);
									MstrResult.add(i1,al);
								}
								else{

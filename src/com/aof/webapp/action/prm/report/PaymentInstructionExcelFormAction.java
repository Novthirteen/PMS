 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class PaymentInstructionExcelFormAction extends ReportBaseAction {
	...
	private ActionForward ExportToExcel (ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response){
		try {
			...
			ProjectPayment pb = (ProjectPayment)hs.load(ProjectPayment.class, new Long(payId));
			if (pb == null) return null;
			
			List CAFList  = pb.getCAFDetails();
			//List AllowanceList = pb.getAllowanceDetails();
			List ExpenseList = pb.getExpenseDetails();
			List AcceptanceList = pb.getPaymentDetails();
		...
		//	double AllowanceAmt = pb.getAllowanceAmount();
			...
			double AcceptanceAmt = pb.getPaymentAmount();
			...
			if (category.equals(Constants.TRANSACATION_CATEGORY_DOWN_PAYMENT)) {
				...
				if (DownPaymentAmt!=0){
					...
					DownPaymentCell.setCellValue(pb.getPaymentCode());
					...
					DownPaymentCell.setCellValue(pb.getPayAddress().getDescription());
					
					DownPaymentsheet.addMergedRegion(new Region(3, (short) 6, 3, (short) 9));
					DownPaymentCell = DownPaymentsheet.getRow(3).getCell((short)6);
					DownPaymentCell.setEncoding(HSSFCell.ENCODING_UTF_16); //设置cell编码解决中文高位字节截断
					DownPaymentCell.setCellValue(pb.getProject().getVendor().getPartyId()+" : "+pb.getProject().getVendor().getDescription());
					
					if (pb.getProject().getParentProject()!=null){
					DownPaymentsheet.addMergedRegion(new Region(4, (short) 2, 4, (short) 7));
					DownPaymentCell = DownPaymentsheet.getRow(4).getCell((short)2);
					DownPaymentCell.setEncoding(HSSFCell.ENCODING_UTF_16); //设置cell编码解决中文高位字节截断
					DownPaymentCell.setCellValue(pb.getProject().getParentProject().getProjId()+" : "+pb.getProject().getParentProject().getProjName());
					
					DownPaymentsheet.addMergedRegion(new Region(5, (short) 2, 5, (short) 7));				//	7
					DownPaymentCell = DownPaymentsheet.getRow(5).getCell((short)2);
					DownPaymentCell.setEncoding(HSSFCell.ENCODING_UTF_16); //设置cell编码解决中文高位字节截断
					DownPaymentCell.setCellValue(pb.getProject().getParentProject().getCustomer().getPartyId()+" : "+pb.getProject().getParentProject().getCustomer().getDescription());
					}
					HSSFRow DownPaymentHRow = null;
					HSSFCellStyle Style5 = DownPaymentsheet.getRow(5).getCell((short)9).getCellStyle();
					if (DownPaymentList != null && DownPaymentList.size() > 0) {
						...
						for (int i0 = 0; i0 < DownPaymentList.size(); i0++) {
							...
							*/
							...
							PaymentTransactionDetail btd = (PaymentTransactionDetail)DownPaymentList.get(i0);
...
//						 footer			
						DownPaymentHRow = DownPaymentsheet.createRow(ExcelRow4+2);
					//	DownPaymentsheet.addMergedRegion(new Region(ExcelRow4+2, (short) 0, ExcelRow4+2, (short) 1));
						DownPaymentCell = DownPaymentHRow.createCell((short)0);
						DownPaymentCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						DownPaymentCell.setCellValue("Prepared By :");
						DownPaymentCell.setCellStyle(Style5);
						DownPaymentsheet.addMergedRegion(new Region(ExcelRow4+2, (short) 1, ExcelRow4+2, (short) 2));
						DownPaymentCell = DownPaymentHRow.createCell((short)1);
						DownPaymentCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						DownPaymentCell.setCellValue(pb.getCreateUser().getName());
						DownPaymentCell.setCellStyle(Style5);
						DownPaymentCell = DownPaymentHRow.createCell((short)3);
						DownPaymentCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						DownPaymentCell.setCellValue("Date :");
						DownPaymentCell.setCellStyle(Style5);
					//	DownPaymentsheet.addMergedRegion(new Region(ExcelRow4+2, (short) 7, ExcelRow4+2, (short) 8));
						DownPaymentCell = DownPaymentHRow.createCell((short)4);
						DownPaymentCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						DownPaymentCell.setCellValue(df.format(new Date()));
						DownPaymentCell.setCellStyle(Style5);
						DownPaymentHRow = DownPaymentsheet.createRow(ExcelRow4+2);
					//	DownPaymentsheet.addMergedRegion(new Region(ExcelRow4+2, (short) 0, ExcelRow4+2, (short) 1));
						DownPaymentCell = DownPaymentHRow.createCell((short)0);
						DownPaymentCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						DownPaymentCell.setCellValue("Approved By :");
						DownPaymentCell.setCellStyle(Style5);
						DownPaymentCell = DownPaymentHRow.createCell((short)3);
						DownPaymentCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						DownPaymentCell.setCellValue("Date :");
					...
					if (CreditDownPaymentList != null && CreditDownPaymentList.size() > 0) {
						...
						for (int i0 = 0; i0 < CreditDownPaymentList.size(); i0++) {
							...
							*/
							...
							PaymentTransactionDetail btd = (PaymentTransactionDetail)CreditDownPaymentList.get(i0);
							DownPaymentHRow = DownPaymentsheet.createRow(ExcelRow4);
							DownPaymentsheet.addMergedRegion(new Region(ExcelRow4, (short)0, ExcelRow4, (short)5 ));
							DownPaymentCell = DownPaymentHRow.createCell((short)0);
							DownPaymentCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
...
//						 footer			
				...
				if(AcceptanceAmt!=0){
					...
					AcceptanceCell.setCellValue(pb.getPaymentCode());
					
					Acceptancesheet.addMergedRegion(new Region(3, (short) 1, 3, (short) 3));
					AcceptanceCell = Acceptancesheet.getRow(3).getCell((short)1);
					AcceptanceCell.setEncoding(HSSFCell.ENCODING_UTF_16); //设置cell编码解决中文高位字节截断
					AcceptanceCell.setCellValue(pb.getPayAddress().getDescription());
					
					Acceptancesheet.addMergedRegion(new Region(3, (short) 6, 3, (short) 9));
					AcceptanceCell = Acceptancesheet.getRow(3).getCell((short)6);
					AcceptanceCell.setEncoding(HSSFCell.ENCODING_UTF_16); //设置cell编码解决中文高位字节截断
					AcceptanceCell.setCellValue(pb.getProject().getVendor().getPartyId()+" : "+pb.getProject().getVendor().getDescription());
					if (pb.getProject().getParentProject()!=null){
					Acceptancesheet.addMergedRegion(new Region(4, (short) 2, 4, (short) 7));
					AcceptanceCell = Acceptancesheet.getRow(4).getCell((short)2);
					AcceptanceCell.setEncoding(HSSFCell.ENCODING_UTF_16); //设置cell编码解决中文高位字节截断
					AcceptanceCell.setCellValue(pb.getProject().getParentProject().getProjId()+" : "+pb.getProject().getParentProject().getProjName());
					
					Acceptancesheet.addMergedRegion(new Region(5, (short) 2, 5, (short) 7));				//	7
					AcceptanceCell = Acceptancesheet.getRow(5).getCell((short)2);
					AcceptanceCell.setEncoding(HSSFCell.ENCODING_UTF_16); //设置cell编码解决中文高位字节截断
					AcceptanceCell.setCellValue(pb.getProject().getParentProject().getCustomer().getPartyId()+" : "+pb.getProject().getParentProject().getCustomer().getDescription());
					}
					...
					if (AcceptanceList != null && AcceptanceList.size() > 0) {
						...
						HSSFCellStyle Style5 = Acceptancesheet.getRow(5).getCell((short)9).getCellStyle();
						
						int AllowanceCount = 0;
						double AllowanceTotal = 0;
						for (int i0 = 0; i0 < AcceptanceList.size(); i0++) {
							PaymentTransactionDetail btd = (PaymentTransactionDetail)AcceptanceList.get(i0);
							AcceptanceHRow = Acceptancesheet.createRow(ExcelRow3);
							Acceptancesheet.addMergedRegion(new Region(ExcelRow3, (short)0, ExcelRow3, (short)2 ));
							AcceptanceCell = AcceptanceHRow.createCell((short)0);
							AcceptanceCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
...
//						 footer			
						AcceptanceHRow = Acceptancesheet.createRow(ListStartRow+AllowanceCount+2);
						//Acceptancesheet.addMergedRegion(new Region(ListStartRow+AllowanceCount+2, (short) 0, ListStartRow+AllowanceCount+2, (short) 1));
						AcceptanceCell = AcceptanceHRow.createCell((short)0);
						AcceptanceCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						AcceptanceCell.setCellValue("Prepared By :");
						AcceptanceCell.setCellStyle(Style5);
						Acceptancesheet.addMergedRegion(new Region(ListStartRow+AllowanceCount+2, (short) 1, ListStartRow+AllowanceCount+2, (short) 2));
						AcceptanceCell = AcceptanceHRow.createCell((short)1);
						AcceptanceCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						AcceptanceCell.setCellValue(pb.getCreateUser().getName());
						AcceptanceCell.setCellStyle(Style5);
						AcceptanceCell = AcceptanceHRow.createCell((short)3);
						AcceptanceCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						AcceptanceCell.setCellValue("Date :");
						AcceptanceCell.setCellStyle(Style5);
				//		Acceptancesheet.addMergedRegion(new Region(ListStartRow+AllowanceCount+2, (short) 7, ListStartRow+AllowanceCount+2, (short) 8));
						AcceptanceCell = AcceptanceHRow.createCell((short)4);
						AcceptanceCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						AcceptanceCell.setCellValue(df.format(new Date()));
						AcceptanceCell.setCellStyle(Style5);
						AcceptanceHRow = Acceptancesheet.createRow(ListStartRow+AllowanceCount+4);
						AcceptanceCell = AcceptanceHRow.createCell((short)0);
						AcceptanceCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
		...
		//	Expense	--------------------------------------------------------------------------
				if(ExpenseAmt!=0){
					...
					ExpenseCell.setCellValue(pb.getPaymentCode());
					...
					ExpenseCell.setCellValue(pb.getPayAddress().getDescription());
					
					Expensesheet.addMergedRegion(new Region(3, (short) 6, 3, (short) 9));
					ExpenseCell = Expensesheet.getRow(3).getCell((short)6);
					ExpenseCell.setEncoding(HSSFCell.ENCODING_UTF_16); //设置cell编码解决中文高位字节截断
					ExpenseCell.setCellValue(pb.getProject().getVendor().getPartyId()+" : "+pb.getProject().getVendor().getDescription());
					if (pb.getProject().getParentProject()!=null){
					Expensesheet.addMergedRegion(new Region(4, (short) 2, 4, (short) 7));
					ExpenseCell = Expensesheet.getRow(4).getCell((short)2);
					ExpenseCell.setEncoding(HSSFCell.ENCODING_UTF_16); //设置cell编码解决中文高位字节截断
					ExpenseCell.setCellValue(pb.getProject().getParentProject().getProjId()+" : "+pb.getProject().getParentProject().getProjName());
					
					Expensesheet.addMergedRegion(new Region(5, (short) 2, 5, (short) 7));				//	7
					ExpenseCell = Expensesheet.getRow(5).getCell((short)2);
					ExpenseCell.setEncoding(HSSFCell.ENCODING_UTF_16); //设置cell编码解决中文高位字节截断
					ExpenseCell.setCellValue(pb.getProject().getParentProject().getCustomer().getPartyId()+" : "+pb.getProject().getParentProject().getCustomer().getDescription());
					}
					...
					if (ExpenseList != null && ExpenseList.size() > 0) {
						...
						HSSFCellStyle Style5 = Expensesheet.getRow(5).getCell((short)9).getCellStyle();
						int ExpenseCount = 0;
						double ExpenseTotal = 0;
						for (int i0 = 0; i0 < ExpenseList.size(); i0++) {
							PaymentTransactionDetail btd = (PaymentTransactionDetail)ExpenseList.get(i0);
...
//						 footer			
						ExpenseHRow = Expensesheet.createRow(ListStartRow+ExpenseCount+2);
						//Expensesheet.addMergedRegion(new Region(ListStartRow+ExpenseCount+2, (short) 0, ListStartRow+ExpenseCount+2, (short) 1));
						ExpenseCell = ExpenseHRow.createCell((short)0);
						ExpenseCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						ExpenseCell.setCellValue("Prepared By :");
						ExpenseCell.setCellStyle(Style5);
						Expensesheet.addMergedRegion(new Region(ListStartRow+ExpenseCount+2, (short) 1, ListStartRow+ExpenseCount+2, (short) 2));
						ExpenseCell = ExpenseHRow.createCell((short)1);
						ExpenseCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						ExpenseCell.setCellValue(pb.getCreateUser().getName());
						ExpenseCell.setCellStyle(Style5);
						ExpenseCell = ExpenseHRow.createCell((short)3);
						ExpenseCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						ExpenseCell.setCellValue("Date :");
						ExpenseCell.setCellStyle(Style5);
						ExpenseCell = ExpenseHRow.createCell((short)4);
						ExpenseCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						ExpenseCell.setCellValue(df.format(new Date()));
						ExpenseCell.setCellStyle(Style5);
						ExpenseHRow = Expensesheet.createRow(ListStartRow+ExpenseCount+4);
						ExpenseCell = ExpenseHRow.createCell((short)0);
						ExpenseCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						ExpenseCell.setCellValue("Approved By :");
						ExpenseCell.setCellStyle(Style5);
						ExpenseCell = ExpenseHRow.createCell((short)3);
						ExpenseCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						ExpenseCell.setCellValue("Date :");
...
//				CAF	---------------------------------------------------------------------------	
				if (CAFAmt!=0){
					...
					CAFsheet.addMergedRegion(new Region(2, (short) 1, 2, (short) 5));
					CAFCell = CAFsheet.getRow(2).getCell((short)1);
					CAFCell.setCellValue(pb.getProject().getProjId() + " : " +pb.getProject().getProjName());
					
					CAFsheet.addMergedRegion(new Region(2, (short) 7, 2, (short) 9));
					CAFCell = CAFsheet.getRow(2).getCell((short)7);
					CAFCell.setCellValue(pb.getPaymentCode());
					
					CAFsheet.addMergedRegion(new Region(3, (short) 1, 3, (short) 3));
					CAFCell = CAFsheet.getRow(3).getCell((short)1);
					CAFCell.setEncoding(HSSFCell.ENCODING_UTF_16); //设置cell编码解决中文高位字节截断
					CAFCell.setCellValue(pb.getPayAddress().getDescription());
					
					CAFsheet.addMergedRegion(new Region(3, (short) 6, 3, (short) 9));
					CAFCell = CAFsheet.getRow(3).getCell((short)6);
					CAFCell.setEncoding(HSSFCell.ENCODING_UTF_16); //设置cell编码解决中文高位字节截断
					CAFCell.setCellValue(pb.getProject().getVendor().getPartyId()+" : "+pb.getProject().getVendor().getDescription());
					if (pb.getProject().getParentProject()!=null){
					CAFsheet.addMergedRegion(new Region(4, (short) 2, 4, (short) 7));
					CAFCell = CAFsheet.getRow(4).getCell((short)2);
					CAFCell.setEncoding(HSSFCell.ENCODING_UTF_16); //设置cell编码解决中文高位字节截断
					CAFCell.setCellValue(pb.getProject().getParentProject().getProjId()+" : "+pb.getProject().getParentProject().getProjName());
					
					CAFsheet.addMergedRegion(new Region(5, (short) 2, 5, (short) 7));				//	7
					CAFCell = CAFsheet.getRow(5).getCell((short)2);
					CAFCell.setEncoding(HSSFCell.ENCODING_UTF_16); //设置cell编码解决中文高位字节截断
					CAFCell.setCellValue(pb.getProject().getParentProject().getCustomer().getPartyId()+" : "+pb.getProject().getParentProject().getCustomer().getDescription());
					}
					...
					if (CAFList != null && CAFList.size() > 0) {
						...
						HSSFCellStyle Style3 = CAFsheet.getRow(ListStartRow).getCell((short)7).getCellStyle();
						HSSFCellStyle Style4 = CAFsheet.getRow(ListStartRow).getCell((short)8).getCellStyle();
						HSSFCellStyle Style5 = CAFsheet.getRow(5).getCell((short)9).getCellStyle();
						int CAFCount = 0;
						double CAFTotal = 0;
						double dayTotal = 0;
						for (int i0 = 0; i0 < CAFList.size(); i0++) {
							PaymentTransactionDetail btd = (PaymentTransactionDetail)CAFList.get(i0);
			...
			// footer			
					...
					//	CAFsheet.addMergedRegion(new Region(ListStartRow+CAFCount+2, (short) 0, ListStartRow+CAFCount+2, (short) 1));
						CAFCell = CAFHRow.createCell((short)0);
						CAFCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						CAFCell.setCellValue("Prepared By :");
						CAFCell.setCellStyle(Style5);
						CAFsheet.addMergedRegion(new Region(ListStartRow+CAFCount+2, (short) 1, ListStartRow+CAFCount+2, (short) 2));
						CAFCell = CAFHRow.createCell((short)1);
						CAFCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断
						CAFCell.setCellValue(pb.getCreateUser().getName());
						CAFCell.setCellStyle(Style5);
						CAFCell = CAFHRow.createCell((short)3);
						CAFCell.setEncoding(HSSFCell.ENCODING_UTF_16);//设置cell编码解决中文高位字节截断

...
var parent = pObj.parentNode;//document.getElementById('D'+id);
var obj = parent.firstChild;
obj = obj.nextSibling;
if(parent.parentNode!=null){
TickParent(parent.parentNode,obj.checked);}

if(parent.nextSibling!=null){
TickChildren(parent.nextSibling,obj.checked);}
}



function TickChildren(tickobj,status)//?div???span
{
	var c = tickobj.id;
	if (c != null) {
		c = c.charAt(0);}
	if(c=='B')
	{
		var array = tickobj.children;//get the children for span
		var obj;
		for(var i=0;i<array.length;i++)
		{
			if(array[i]!=null)
			{
				if(array[i].type == 'branchroot')//div
				{
					obj = array[i];
					obj.firstChild.nextSibling.checked = status;
				}
				else
				{
					TickChildren(array[i],status);
				}
			}
		}
	}
...
function TickParent(tickobj,status){//??div?parent

	var arr = tickobj.children;
	var flag = 1;
	var obj;
	if(status == true){//parent should be ticked too
	
		obj = tickobj;
		if(obj.id!='treeroot'){
		
			obj = obj.previousSibling;
			obj.firstChild.nextSibling.checked = true;
			
			TickParent(obj.parentNode,status);
		}
	}else{ //if all the children is not ticked, then parent is unticked
	
		for(var i=0;i<tickobj.children.length;i++){
			if(arr[i].nodeName=='DIV'){
				obj = arr[i].firstChild.nextSibling;
		...
		if(obj.id!='treeroot'){
			obj = obj.previousSibling;
			obj.firstChild.nextSibling.checked = false;
...
XmlTree.prototype.addNode = function(id, desc, parentId) {
	...
	hid1.type = 'hidden'
	hid1.name = 'parent'+id;
	node.appendChild(hid1);

	var hid2 = document.createElement("input")
	hid2.type = 'hidden'
	hid2.name = 'desc'+id;
	hid2.value = desc;
	node.appendChild(hid2);
...
XmlTree.prototype.EditNodeDesc = function EditNode(ranking,id,desc)
{
	var obj = this.container.all('N' + ranking);
	if (obj == null) return;
		obj.innerText = desc;

//	obj = this.container.all('D' + ranking);
//	if (obj == null) return;
//	else
//		obj.innerText = desc;

	obj = this.container.all('desc'+ranking);
	if(obj!=null)obj.value = ranking;
	
//	var bobj = this.container.all('B' + ranking);

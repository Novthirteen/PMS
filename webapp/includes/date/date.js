  ...
  if(eD==g_eC&&"block"==dF.style.display){
    ...
    if(GetInputDate(eD.value,g_dFmt)){
	  ...
	  wF.SetInputDate(-1,-1,-1);
			  if((eD!=null)&&(eD.value.length>0))
			  {
			  	wF.query = eD.value
			  	var query = "";
				query = wF.query;
				var arrDate = query.split("-");
				var year = parseInt(arrDate[0]);
				var month =parseInt(arrDate[1]);
				if((arrDate[1].length>1)&&(arrDate[1].substring(0,1)=='0'))
					month = parseInt(arrDate[1].substring(1,2))
					
				var date =parseInt(arrDate[2]);
				if((arrDate[2].length>1)&&(arrDate[2].substring(0,1)=='0'))
					date = parseInt(arrDate[2].substring(1,2))

				wF.SetDate(date,month,year,1);
				}
...
function LoadMonths(n){
  ...
  for(i=0;i<n;i++){
    ...
    rg[i].src="w"+GetDOW2(1,m,y)+"d"+GetMonthCount(m,y)+".gif";
    m++;
	if(12<m){ m=1;y++;}
  }
}

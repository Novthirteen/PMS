...
<script language="javascript">
	...
	function showCustomerGroupDialog(){
		...
		if (v != null) {
			code=v.split("|")[0];
			document.getElementById("AccountId").value=code;
			desc=v.split("|")[1];
			document.getElementById("name").innerHTML=desc;
		}
	}
	function editCustomerGroupDialog(){
	var id = document.getElementsByName("AccountId");
		var code,desc;
		v = window.showModalDialog(
			"system.showDialog.do?title=helpdesk.servicelevel.category.dialog.title&editCustomerGroup.do"
			+"?openType=dialogView&DataId="+id[0].value,
			null,
			'dialogWidth:560px;dialogHeight:660px;status:no;help:no;scroll:no');
		if (v != null) {
			code=v.split("|")[0];
      ...
      <table width='100%' border='0' cellspacing='0' cellpadding='0'>
        <tr>
          <TD align=left width='90%' class="wpsPortletTopTitle">
            Prospect Customer Verification
          </TD>
        </tr>
      </table>
    </TD>
    ...
    <table width='100%' border='0' cellpadding='0' cellspacing='2'>
     ...
     <td align="left">
      ...
      <tr>
        ...
        <td align="left">
		<input type="button" value="<bean:message key="button.save"/>" class="loginButton" onclick="fnSubmit('update')"/>
		<% if((CustParty.getStatus()==null)||(CustParty.getStatus().equalsIgnoreCase("unverified"))){ %>
		<input type="button" value="Save as Customer" class="loginButton" onclick="fnSubmit('verify')"/>
		<%}else if((CustParty.getStatus()!=null)||(CustParty.getStatus().equalsIgnoreCase("verified"))){ %>
		<input type="button" value="UnVerify" class="loginButton" onclick="fnSubmit('unverify')"/>
		<%}%>
        </td>
		<td>&nbsp;</td>
		<td><input type="button" value="Back To List" class="loginButton" onclick="location.replace('listProspect.do')"></td>
      </tr>
    </table>
 	</form>

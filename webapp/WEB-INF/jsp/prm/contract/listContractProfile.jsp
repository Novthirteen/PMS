...
<tr>
	...
	<TABLE width="100%">
		...
		<tr>
			...
			<td class="lblbold">Contract No. or Description:</td>
			...
			<td class="lblLight"><input type="text" name="textCustomer" size="15" value="<%=textCustomer%>" style="TEXT-ALIGN: right" class="lbllgiht"></td>
	    ...
	    <tr>
	     ...
	     <td class="lblbold">Contract Status:</td>
		    <td class="lblLight">
		    <select name="textStatus" >
   	            <option value="" selected>ALL</option>
				<option value="Signed" <%if (textStatus.equals(Constants.CONTRACT_PROFILE_STATUS_SIGNED)) out.print("selected");%>>Signed</option>
				<option value="Unsigned" <%if (textStatus.equals(Constants.CONTRACT_PROFILE_STATUS_UNSIGNED)) out.print("selected");%>>Unsigned</option>
				<option value="Cancel" <%if (textStatus.equals(Constants.CONTRACT_PROFILE_STATUS_CANCEL)) out.print("selected");%>>Cancel</option>
				<option value="Closed" <%if (textStatus.equals(Constants.CONTRACT_PROFILE_STATUS_CLOSED)) out.print("selected");%>>Closed</option>
		    </select>
	     </td>
	   ...
	   <tr>
	    ...
	    <tr>
	    	...
	    	<td  align="middle" colspan=2>
			    ...
			    <input type="button" value="New" class="button" onclick="location.replace('editContractProfile.do')">
...
<TABLE border=0 width='100%' cellspacing='2' cellpadding='2' class=''>
  <TR bgcolor="#e9eee9">
    ...
    <td align="center" class="lblbold">Customer Sat.</td>
  </tr>
  <%
  	String newCustomer = "";
  	String oldCustomer = "";
	%>
  <logic:iterate id="p" indexId="indexId" offset="offset" length="length" name="QryList" type="com.aof.component.prm.contract.ContractProfile" >                
  <tr bgcolor="#e9eee9">  
    <td align="left"><%=i++%></td>
    <td align="left" class="lblbold">
        ...
        <p >
        <%
        	newCustomer = p.getCustomer().getDescription();
        %>
        <%= newCustomer%>
        </div>
    </td>
    <td align="left"> 
		<a href="editContractProfile.do?FormAction=view&contractId=<bean:write name="p" property="id"/>"><bean:write name="p" property="no"/></a> 
    </td>
    ...
    <td align="center"> 
      <div class="head4">                   
        <p ><%=p.getLegalReviewDate() != null ? dateFormater.format(p.getLegalReviewDate()) : ""%>
         ...
         <td align="center">
     <div class="head4">                   
        <p ><%=p.getCustomerSat() != null ? p.getCustomerSat() : ""%>
      </div>
     </td>
				</tr>
				</logic:iterate> 		

<tr align="center" bgcolor="#e9eee9">
	<td class="lblbold" align="center">&nbsp;
		<%
			if (canUpdate) {
		%>
		<input type="checkbox"  class="checkboxstyle" name="chkAllPay" onclick="checkUncheckAllBox(document.editForm.chkAllPay, document.editForm.payDetailId)">
		<%
			} else {
		%>
		#
...
<tr align="center" bgcolor="#e9eee9">
	<TD align="center">
		<%
			if (canUpdate) {
		%>
		<input type="checkbox" class="checkboxstyle" name="payDetailId" value="<%=btd.getTransactionId()%>" onclick="checkTopBox(document.editForm.chkAllPay, document.editForm.payDetailId)">
		<%
			} else {
		%>
			<%=i0 + 1%>
